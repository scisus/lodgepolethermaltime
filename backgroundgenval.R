...
partest <- read.csv("../phd/data/OrchardInfo/ParentTrees/ParentTreeExtractReport_Tests_2014_05_15_13_38_17.csv") %>% select(Parent.Tree.Number, Genetic.Quality.Value) %>%
  rename(Clone = Parent.Tree.Number) %>%
  mutate(Clone = as.character(Clone))

parentdat <- read.csv("../phd/data/OrchardInfo/ParentTrees/parents.csv") %>%
  select(Clone = Parent.Tree.Number, Latitude, Longitude, Elevation) %>%
  mutate(Clone = as.character(Clone))
...
phenf <- full_join(phenfpar, phenfprov) %>%
  ...
  left_join(partest) %>%
  filter(!is.na(Genetic.Quality.Value)) # drop NAs for now
...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + Latitude + Latitude*Elevation + MAT + Genetic.Quality.Value + (1|Provenance) + (1|Site) + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginGenval.stan",
             file = "female_beginGenval",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitgenval",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_latgenval.rds")
