...
ggplot(variation, aes(y = fct_rev(.variable), x = .value, colour = .variable, linetype = Sex)) +
  ...
  scale_colour_viridis_d() +
  labs(title = "Standard deviation of population mean and offsets", caption = "2000 draws from the posterior") +
  ylab("") +
  xlab("Forcing") +
  facet_grid(event ~ .) +
...
ggplot(doypredmatchfut_medians, aes(y = Date, x = normal_period, ymin = .lowerdate, ymax = .upperdate, group = interaction(normal_period, Sex), colour = Sex)) +
