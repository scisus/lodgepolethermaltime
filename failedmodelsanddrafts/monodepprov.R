...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + (1|Site) + (1|Clone) + (1|Year) + (1|Tree) + Latitude + EMT + MAT + (mo(Generation)|Provenance))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginMonodepprov.stan",
             file = "female_beginMonodepprov",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitMonodepprov",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_latMonodepprov.rds")
