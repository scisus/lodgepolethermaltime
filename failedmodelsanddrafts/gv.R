...
pargv <- read.csv("../phd/data/OrchardInfo/ParentTrees/ParentTreeExtractReport_Tests_2014_05_15_13_38_17.csv") %>%
  select(Clone = Parent.Tree.Number, Genetic.Quality.Value)
...
fbdat <- filter_sex_event(sex = "FEMALE", event = "begin", phenf) %>%
  filter(!is.na(Genetic.Quality.Value))
...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + Genetic.Quality.Value + Latitude + MAT + EMT + SHM + (1|Site) + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_begingv.stan",
             file = "female_begingv",
             prior = bprior,
             inits = initpars,
             iter = niter,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitgv",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_gv.rds")
