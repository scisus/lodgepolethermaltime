...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + (1|Site) + bFFP + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginbFFP.stan",
             file = "female_beginbFFP",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitbFFP",
...
saveRDS(loo_fbfit, "model_dev/loo_bffp.rds")
