...
phenf$SourceLoc <- phenf %>% group_by(Latitude, Longitude, Elevation) %>% group_indices()
phenf$SourceLoc <- as.character(phenf$SourceLoc)
...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + (1|Site) + (1|Clone) + (1|Year) + (1|Tree) + (1|SourceLoc))
...
fbfit <- brm(bform, data = fbdat,
             save_model = paste0("female_begin_", mn, ".stan"),
             file = paste0("female_begin", mn),
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = paste0("fbfit", mn),
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, paste0("model_dev/loo_", mn, ".rds"))
