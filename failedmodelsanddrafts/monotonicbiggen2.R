...
parentdat <- read.csv("../phd/data/OrchardInfo/ParentTrees/parents.csv") %>%
  select(Clone = Parent.Tree.Number, Elevation, Latitude, Longitude) %>%
...
phenf <- full_join(phenfpar, phenfprov) %>%
  ...
  filter(! is.na(Elevation)) #drop missing elevation for now
...
initpars <- lapply(1:6, function(id) list(sigma = 30, Intercept = 300)) #consider sigma 10

# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + Latitude*Elevation + MAT + (1|Provenance) + (1|Site) + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginbigGen2.stan",
             file = "female_beginbigGen2",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitbiggen2",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_latbiggen.rds2")
