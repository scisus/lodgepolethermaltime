...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginbase.stan",
             file = "female_beginbase",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, reloo = TRUE, model_names = "fbfitbase",
                 reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                                  prior("normal(0,15)", class = "sigma"),
                                                                  prior("normal(0,9)", class = "sd"),
...
saveRDS(loo_fbfit, "model_dev/loo_base.rds")
