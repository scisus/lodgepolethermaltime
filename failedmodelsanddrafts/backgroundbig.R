...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + (1|Site) + EMT + MAT + SHM + Latitude + (1|Provenance) + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginbig.stan",
             file = "female_beginbig",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitbig",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_big.rds")
