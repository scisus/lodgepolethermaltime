...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + Latitude + EMT + MAT + SHM + mo(Generation) + (1|Provenance) + (1|Site) + (1|Clone) + (1|Year) + (1|Tree) + (1|Orchard))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginmonoparorchprov.stan",
             file = "female_beginmonoparorchprov",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitmonoparorchprov",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.95)))
saveRDS(loo_fbfit, "model_dev/loo_monoparorchprov.rds")
