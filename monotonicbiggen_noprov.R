...
# model formula
bform <- brmsformula(sum_forcing | cens(censored, upper) ~ 1 + Latitude + EMT + MAT + SHM + mo(Generation) + (1|Site) + (1|Clone) + (1|Year) + (1|Tree))
...
fbfit <- brm(bform, data = fbdat,
             save_model = "female_beginbigGennp.stan",
             file = "female_beginbigGennp",
             prior = bprior,
             inits = initpars,
             iter = niter,
             cores = ncores,
...
loo_fbfit <- loo(fbfit, model_names = "fbfitbiggennp",
                 reloo = TRUE, reloo_args = list(prior = c(prior("normal(400,100)", class = "Intercept"),
                                                 ...
                                                 control=list(adapt_delta = 0.9)))
saveRDS(loo_fbfit, "model_dev/loo_latbiggennp.rds")
