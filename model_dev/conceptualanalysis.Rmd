...
ggplot(ff, aes(x=DoY, y=sum_forcing, colour = interaction(Site,Year))) +
  geom_line() +
  theme(legend.position = "none") +
  ggtitle("forcing (ristos) accumulated at Day of Year", subtitle = "May 1 - June 30")
```
...
fextreme <- quantile(fortforc$fortforc, 0.99)

feextreme <- quantile(fortforce$fortforce, 0.99)

```

The amount of forcing that can accumulate over any two week period between January and June can be quite large - 99th percentile is `r fextreme`. For the start period, which is earlier, it's a bit smaller - `r feextreme`. The sd required to obtain these values is
...
pnorm(fextreme, 0, sd=80) # sd to cap at extreme
pt(fextreme, 1)
hist(rnorm(1000, 0, sd=80), breaks=50) 

pnorm(feextreme, 0, sd = 55)
pt(feextreme, 1)
...
```
It must be positive and continuous. Values less ~50 and greater than ~1000 would be quite unbelievable for any phase.
...
```{r}
pnorm(ffs99, mean = (ffs1+ffs99)/2, sd = 85) #start
pt(ffs99, 25, ncp = 335)
hist(rnorm(1000, 335, 85), breaks=60)
hist(rt(1000, 25, ncp = 335), breaks=60)
...
```{r}
pnorm(ffe99, mean = (ffe1 + ffe99)/2, sd = 140) #end
pt(ffe99, 25, ncp = 555)
hist(rnorm(1000, 555, 140), breaks=60)
```
...
Looks like Clone, Orchard, and Site might need to have non-centering *within* factors. 
