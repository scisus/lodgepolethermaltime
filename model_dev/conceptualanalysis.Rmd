...
Sigma is likely to be relatively large because of lackadaisical sampling and compounding errors through censoring, multiple observers, and errors in accumulated forcing measurements. I think an extreme upper limit is probably about 2 weeks of forcing. 
...
maxforc <- janjun %>%
  group_by(Site, Year) %>%
  summarise(maxforc = rollsum(forcing, 14))

ggplot(maxforc, aes(x=maxforc)) +
  geom_histogram() +
  ggtitle("Forcing accumulation over 14 day periods Jan-Jun") +
  geom_vline(xintercept = mean(maxforc$maxforc))

# 90th percentile of forcing diffs
dextreme <- quantile(maxforc$maxforc, 1)

pnorm(dextreme, 0, sd=50) # sd to cap at extreme
pexp(dextreme, 0.01)


hist(rnorm(1000, 0, sd=50), breaks=50) 
abline(v = dextreme)
hist(abs(rnorm(1e3, 0, 50)), breaks=50) 
abline(v=dextreme)
