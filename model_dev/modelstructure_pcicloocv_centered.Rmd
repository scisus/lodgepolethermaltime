...
fedat$TreeID <- paste0(fedat$Orchard, fedat$X, fedat$Y)
...
if (freshrun == TRUE) {
  # mean only
  mo <- brm(sum_forcing ~ 0 + Intercept, data = fedat,
            prior = c(prior("normal(350,50)", class = "b"),
  ...
  moc <- brm(sum_forcing | cens(censored_lronly) ~ 0 + Intercept, data = fedat,
             prior = c(prior("normal(350,50)", class = "b"),
                       prior("normal(0,20)", class = "sigma")),
             cores = 5, inits = init_ll)
  ...
  mocf <- brm(sum_forcing | cens(censored, upper) ~ 0 + Intercept, data = fedat,
              prior = c(prior("normal(350,50)", class = "b"),
                        prior("normal(0,20)", class = "sigma")),
              cores=5,  inits = init_ll)
  ...
  ec <- brm(sum_forcing | cens(censored, upper) ~ 0 + Intercept + (1|Site) + (1|Provenance) + (1|Clone) + (1|Year), data = fedat,
            prior = c(prior("normal(350,50)", class = "b"),
                      prior("normal(0,20)", class = "sigma"),
  ...
  ect <- brm(sum_forcing | cens(censored, upper) ~ 0 + Intercept + (1|Site) + (1|Provenance) + (1|Clone) + (1|Year) + (1|TreeID), data = fedat,
            prior = c(prior("normal(350,50)", class = "b"),
                      prior("normal(0,30)", class = "sigma"),
                      prior("normal(0,20)", class = "sd")),
            cores = 5, inits = init_ll, iter = 3000,
            save_pars = save_pars(all = TRUE))
  saveRDS(ect, "model_dev/ect.rds") } else {
...
if (freshrun == TRUE) {
  ...
  loo_ect <- loo(ect, reloo = TRUE, reloo_args = list(prior = c(prior("normal(350,50)", class = "b"),
                                                                prior("normal(0,30)", class = "sigma"),
                                                                prior("normal(0,20)", class = "sd")),
                 future = TRUE,
                 inits = init_ll, iter = 3000))
