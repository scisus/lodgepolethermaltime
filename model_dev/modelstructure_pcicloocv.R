...
climdat <- "data/all_clim_PCIC.csv"
spudat <- "data/LodgepoleSPUs.csv"

phenf <- prepare_data(phendat, forcingtype = "gdd", clim = climdat, spu = spudat)
...
if (freshrun == TRUE) {
  ...
  ect <- brm(sum_forcing | cens(censored, upper) ~ 1 + (1|Site) + (1|Provenance) + (1|Clone) + (1|Year) + (1|TreeID), data = fedat,
            ...
            save_pars = save_pars(all = TRUE))
  saveRDS(ect, "ect.rds") } else {
    mo <- readRDS("mo.rds")
    moc <- readRDS("moc.rds")
    mocf <- readRDS("mocf.rds")
    ec <- readRDS("ec.rds")
    ect <- readRDS("ect.rds")
  }
