...
fit <- rstan::stan(file='meanfitrealsiteyearprovclone_censor.stan', chains=5, data=input, iter=1e4, control = list(adapt_delta=0.99, max_treedepth=11), cores=5, pars=c("z_clone_offset"), include=FALSE, init = 
                       list(list(sum_forcing_true = rnorm(2608, 200,10), sum_forcing_obs = rnorm(2608, 500,10)),
                            list(sum_forcing_true = rnorm(2608, 200,10), sum_forcing_obs = rnorm(2608, 500,10)),
                            list(sum_forcing_true = rnorm(2608, 200,10), sum_forcing_obs = rnorm(2608, 500,10)),
                            list(sum_forcing_true = rnorm(2608, 200,10), sum_forcing_obs = rnorm(2608, 500,10)),
                            list(sum_forcing_true = rnorm(2608, 200,10), sum_forcing_obs = rnorm(2608, 500,10))))
...
# fit <- readRDS("2020-08-21sypc_censorFEMALE.rds")
# 
# print(fit)
# 
# library(shinystan)
